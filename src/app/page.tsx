...
export default function Home() {
  ...
  async function fetchVideos(pageNumber: number, reset = false) {
    ...
    const userIds = Array.from(
      new Set(normalizedVideos.map((v) => v.user_id).filter(Boolean))
    ) as string[]
      ...
      if (profileError) {
        ...
        profileRows.forEach((row: any) => {
          if (row?.id && row?.display_name) {
            profileMap[row.id] = row.display_name
          }
        })
    ...
    if (normalizedVideos.length < PAGE_SIZE) {
      setHasMore(false)
    }
    ...
    if (reset) {
      ...
      setVideos((prev) => [...prev, ...normalizedVideos])
  ...
  async function toggleLike(video: any) {
    ...
    if (existingError) {
      console.error("Error checking like:", existingError)
      return
    }
